...
INSTRUCTIONS = """The user's question will appear as shown:
...
Valid "action" values are strings: "Final Answer" or {tool_names}
...
The results of tool invocations will appear as "Observations". \
...
Respond to the user by taking an action with name "Final Answer" - in this case the "action_input" is what you will respond to the user.

You can only take a single action at a time.

Putting it altogether, the full sequence to solve the user input will look like:
...
(This action/observation repeats as many times as needed)
<action>
{{
  "action": "Final Answer",
  "action_input": ...
...
if __name__ == "__main__":
    ...
    def get_chain():
        ...
        prompt = PromptTemplate.from_template(TEMPLATE).partial(dhead=df.head().to_markdown(),
                                                                **generate_tool_info([repl, retriever_tool]))

        chain = {
            "input": lambda x: x["input"],
            "agent_scratchpad": lambda x: construct_scratchpad(x["intermediate_steps"])
                }|prompt | VertexAI(temperature=0, max_output_tokens=1000).bind(stop=["<observation>"]) | _decode
        agent = RunnableAgent(runnable=chain, specified_input_keys=["input"])
    ...
    chain_results = run_on_dataset(
        ...
        project_name=f"Vertex LLM {time.time()}"
    )
